...
from model.srel_inter import SREL_rep_mu
...
def main(save_weights, save_logs, save_mat, lambda_eta,weightdir):
    ...
    if weightdir:
        ...
        dir_dict_saved = (
            'weights/intra_phase2/rep_eta/'
            '20240422-122428_Nstep10_batch05_sinr_8.07dB')
        loaded_dict = torch.load(os.path.join(dir_dict_saved,'model_with_attrs.pth'), 
                                 map_location=device)
    ...
    model_intra_phase2 = SREL_rep_eta(constants)
    model_intra_phase2.load_state_dict(loaded_dict['state_dict'])   
    
    # freeze model_intra
    for param in model_intra_phase2.parameters():
        param.requires_grad = False
        
    # Initialize model
    model_inter = SREL_rep_mu(constants, model_intra_phase1)
